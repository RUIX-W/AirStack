...
```bash
docker exec airstack-robot-1 bash -c "bws --packages-select <bringup_pkg> my_planner"
```
...
```bash
# 1. Rebuild
docker exec airstack-robot-1 bash -c "bws --packages-select <pkg>"

# 2. Confirm the file was installed
docker exec airstack-robot-1 bash -c "ls install/<pkg>/share/<pkg>/launch/"

# 3. Dry-run launch to catch XML/syntax errors
docker exec airstack-robot-1 bash -c "sws && ros2 launch <pkg> <pkg>.launch.xml --print"

# 4. Actually launch and watch logs
docker exec airstack-robot-1 bash -c "sws && ros2 launch <pkg> <pkg>.launch.xml"

# 5. Verify the node and its topics resolved correctly
docker exec airstack-robot-1 bash -c "ros2 node list"
docker exec airstack-robot-1 bash -c "ros2 node info /<robot_name>/<namespace>/<node>"
docker exec airstack-robot-1 bash -c "ros2 topic info /<robot_name>/<expected_topic>"
```
...
- **Real launch files to study:**
  - Module: `robot/ros_ws/src/autonomy/0_interface/px4_interface/launch/px4_interface.launch.xml`
  - Module: `robot/ros_ws/src/autonomy/3_local/b_planners/droan_local_planner/launch/droan_local_planner.launch.xml`
  - Layer bringup (global): `robot/ros_ws/src/autonomy/4_global/global_bringup/launch/global.launch.xml`
  - Layer bringup (local): `robot/ros_ws/src/autonomy/3_local/local_bringup/launch/local.launch.xml`
...
- **ROS 2 docs:**
  ...
  - [Using ROS 2 Launch For Large Projects](https://docs.ros.org/en/humble/Tutorials/Intermediate/Launch/Using-ROS2-Launch-For-Large-Projects.html)
