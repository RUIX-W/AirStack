...
**Location:** `simulation/isaac-sim/sitl_integration/<your_scene_name>.py`

```bash
cd simulation/isaac-sim/sitl_integration/
...
Update the main `.env` file to use your scene. The primary scene configuration uses a USD file path:

```bash
# Specify your scene USD path (Nucleus or local)
ISAAC_SIM_SCENE="omniverse://your-nucleus/path/to/your_scene.usd"
```

Alternatively, override from command line:
```bash
ISAAC_SIM_SCENE="omniverse://your-nucleus/path/to/your_scene.usd" airstack up isaac-sim
```
...
**File:** `simulation/isaac-sim/sitl_integration/your_scene_name.md`
...
```bash
# Launch scene (set scene in .env ISAAC_SIM_SCENE or pass via environment)
airstack up isaac-sim
...
**Note:** A `scene_prep.py` utility module may need to be created if it does not already exist in `simulation/isaac-sim/sitl_integration/`. Check if one is available at `simulation/isaac-sim/sitl_integration/scene_prep.py` or similar location before writing your own.

Four reusable helpers that cover the most common environment setup tasks.
...
- **AirStack SITL Integration:**
  - `simulation/isaac-sim/sitl_integration/` — launch scripts and SITL configuration
  - `simulation/isaac-sim/assets/scenes/` — USD scene assets
