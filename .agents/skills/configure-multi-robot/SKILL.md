...
- Familiarity with the AirStack docker-compose layout (`.env`, `robot/docker/`, `simulation/isaac-sim/`)
...
So `airstack-robot-1` → `ROBOT_NAME=robot_1`, `ROS_DOMAIN_ID=1`. Replica `2` → `robot_2`, domain `2`. Etc. **The container name is the source of truth in simulation.**

The top-level [`autonomy_bringup/launch/robot.launch.xml`](../../../robot/ros_ws/src/autonomy/autonomy_bringup/launch/robot.launch.xml) then pushes this name as the root namespace for every node it spawns:
...
```bash
airstack up
docker exec airstack-robot-1 bash -c 'echo $ROBOT_NAME $ROS_DOMAIN_ID'
# robot_1 1
```
...
```bash
docker exec -e ROBOT_NAME=robot_5 -e ROS_DOMAIN_ID=5 -it airstack-robot-1 bash
```
...
So `NUM_ROBOTS=3 airstack up` produces **three** robot containers (`airstack-robot-1`, `-2`, `-3`), each with its own `ROBOT_NAME` and its own `ROS_DOMAIN_ID`. Each container runs the full autonomy stack independently. Cross-robot communication, when needed, goes through the DDS router (see [`onboard_all/config/dds_router.yaml`](../../../robot/ros_ws/src/autonomy/autonomy_bringup/onboard_all/config/dds_router.yaml)) which bridges allowlisted topics from each per-robot domain into a shared GCS domain.

```bash
NUM_ROBOTS=3 airstack up
docker ps --format '{{.Names}}' | grep robot
# airstack-robot-1
# airstack-robot-2
# airstack-robot-3
```
...
[`autonomy_bringup`](../../../robot/ros_ws/src/autonomy/autonomy_bringup/) ships two layouts, selected by the `role` arg / `AUTONOMY_ROLE` env var:
...
The domain bridge config for cross-robot communication is in [`robot/ros_ws/src/robot_bringup/params/domain_bridge.yaml`](../../../robot/ros_ws/src/robot_bringup/params/domain_bridge.yaml).
...
Isaac Sim reads `NUM_ROBOTS` from the environment and spawns matching vehicles named `robot_1`, `robot_2`, … so the names line up with what the resolver assigns to robot containers.

### Isaac Sim (Pegasus)

The `ISAAC_SIM_SCENE` variable in `.env` specifies the USD scene to load. For multi-robot scenarios, spawn scripts in `simulation/isaac-sim/sitl_integration/` read `NUM_ROBOTS` and spawn one drone per robot in a loop. Each drone is created with `robot_name=f"robot_{index}"`, `vehicle_id=index`, `domain_id=index`, and an X offset for spacing:
...
System tests in [`tests/`](../../../tests/) address robots as `/robot_{n}/...` and use `ROS_DOMAIN_ID=n` matching what the resolver assigned inside the container.
...
```bash
for c in $(docker ps --format '{{.Names}}' | grep robot); do
...
- [ ] Verified end-to-end: `NUM_ROBOTS=3 airstack up`, then `docker exec airstack-robot-2 bash -c 'ros2 topic list | grep robot_2'` shows the same topics that `airstack-robot-1` shows under `robot_1`
...
docker exec -e ROS_DOMAIN_ID=1 airstack-robot-1 bash -c \
  "source /opt/ros/humble/setup.bash && ros2 node list"
docker exec -e ROS_DOMAIN_ID=2 airstack-robot-2 bash -c \
  "source /opt/ros/humble/setup.bash && ros2 node list"

# Robots talking to each other? Use the GCS domain (0) and check the router-bridged topics
docker exec airstack-gcs-1 bash -c \
  "source /opt/ros/humble/setup.bash && ROS_DOMAIN_ID=0 ros2 topic list | grep -E 'robot_[0-9]+'"
...
- [`robot/ros_ws/src/robot_bringup/launch/robot.launch.xml`](../../../robot/ros_ws/src/robot_bringup/launch/robot.launch.xml) — top-level robot launch
- [`robot/ros_ws/src/autonomy/autonomy_bringup/launch/autonomy.launch.xml`](../../../robot/ros_ws/src/autonomy/autonomy_bringup/launch/autonomy.launch.xml) — autonomy stack launch with `push_ros_namespace`
- [`robot/ros_ws/src/robot_bringup/params/domain_bridge.yaml`](../../../robot/ros_ws/src/robot_bringup/params/domain_bridge.yaml) — cross-domain bridge config
- [`simulation/isaac-sim/sitl_integration/`](../../../simulation/isaac-sim/sitl_integration/) — Isaac Sim SITL scripts
