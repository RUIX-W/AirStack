...
Rebuild `task_msgs`:
```bash
docker exec airstack-robot-1 bash -c "bws --packages-select task_msgs"
```

### 1. Create the package

Follow [add-ros2-package](../add-ros2-package) to create the package under the appropriate layer (e.g., `robot/ros_ws/src/autonomy/4_global/b_planners/your_executor/`).
...
docker exec airstack-robot-1 bash -c "sws && ros2 action list"
...
`robot/ros_ws/src/autonomy/4_global/b_planners/random_walk/` — implements `ExplorationTask.action`.
