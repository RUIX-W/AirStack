...
- Start, stop, or inspect AirStack services (robot, isaac-sim, gcs, docs)
...
- Loads `.env`, propagates host env overrides, and applies the right include set from
  the top-level `docker-compose.yaml` (isaac-sim / robot / gcs / docs).
- Resolves Compose **profiles** from `COMPOSE_PROFILES` in `.env` automatically.
...
AUTOLAUNCH=false NUM_ROBOTS=2 airstack up robot isaac-sim
```
...
| `ISAAC_SIM_SCENE`            | USD scene path (Nucleus or local) for Isaac Sim           |
...
`PROJECT_NAME="airstack"`: `airstack-robot-1`, `airstack-isaac-sim-1`,
`airstack-gcs-1`, `airstack-docs-1`. With `NUM_ROBOTS=2` you
also get `airstack-robot-2`. Always confirm with `airstack status` or
...
docker exec airstack-robot-1 bash -c "sws && ros2 topic list | grep odom"

# Echo a topic exactly once (will exit cleanly)
docker exec airstack-robot-1 bash -c "ros2 topic echo /robot_1/odometry --once"
...
docker exec airstack-robot-1 bash -c "sws && ros2 run my_package my_node"
```
...
```bash
# Last 100 lines
docker logs --tail 100 airstack-robot-1

# Stream and grep for errors
docker logs -f airstack-robot-1 2>&1 | grep -iE "error|fail|crash"

# Save full log for offline analysis
docker logs airstack-robot-1 > /tmp/robot.log 2>&1
```
...
```bash
docker exec airstack-robot-1 bash -c "ls -la ~/.ros/log/latest/"
docker exec airstack-robot-1 bash -c "tail -100 ~/.ros/log/latest/<node>.log"
```

### Live container resource usage

```bash
docker stats --no-stream airstack-robot-1
```
...
3. **Running `ros2` on the host**
   ...
   - Fix: every `ros2 ...` invocation must be wrapped in
     `docker exec airstack-robot-1 bash -c "..."`. Host has no AirStack overlay.
...
airstack up robot                        # Start one service
AUTOLAUNCH=false airstack up robot       # Start idle (for development) — IMPORTANT
...
airstack logs robot                      # Tail logs (partial name OK)

# ---- Exec inside container (NEVER use -it) ----
docker exec airstack-robot-1 bash -c "ros2 node list"
docker exec airstack-robot-1 bash -c "sws && ros2 topic list"
docker exec airstack-robot-1 bash -c "timeout 5 ros2 topic hz /robot_1/odometry"
docker exec airstack-robot-1 bash -c "ros2 topic echo /robot_1/odometry --once"
...
# ---- Logs ----
docker logs --tail 100 airstack-robot-1
docker logs -f airstack-robot-1 2>&1 | grep -iE "error|fail"

# ---- Other ----
airstack docs                                    # Build + serve MkDocs
airstack test -v                                 # Run system tests
airstack lint                                    # Lint
